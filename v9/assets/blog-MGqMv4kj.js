...
import{t as e}from"./vendor-BDh6mtVu.js";import{S as t,i as n}from"./widev-vc3w_AsM.js";import{t as r}from"./preload-helper-TQeha_t2.js";import{catInfo as i,clearBlogCache as a,fade as o,getPosts as s,prefetchPost as c,skCard as l,srcBadge as u}from"./devblog-rqNSmnhG.js";var d=9,f=[{id:`nuevo`,icon:`fa-clock`,label:`Recientes`},{id:`vistas`,icon:`fa-fire`,label:`Populares`}],p=`wi_blog_cats`,m=(e,n)=>{let r=i(e.categoria);return`
  <article class="bl_card bl_fade" style="--d:${n*.05}s" data-slug="${e.slug||e.id}">
    <div class="bl_card_img">
      ...
      <div class="bl_card_over">
        ...
        ${e.destacado?`<span class="bl_card_dest" ${t(`Destacada`)}><i class="fas fa-star"></i></span>`:``}
      </div>
    </div>
  ...
  </div>`,g=async()=>{let t=`todo`,f=`nuevo`,h=0,g=[],_=[],v=!1,y=e=>r(()=>import(`./rutas-Duz3-mL_.js`).then(e=>e.r).then(t=>t.rutas.navigate(e)),__vite__mapDeps([0])),b=n=>{let r=[...new Set(n.map(e=>e.categoria).filter(Boolean))].sort();localStorage.setItem(p,JSON.stringify(r));let a=e(`#bl_cats`);a.html(`<button class="bl_cat_btn ${t===`todo`?`active`:``}" data-cat="todo" style="--cc:var(--mco)"><i class="fas fa-grip"></i><span>Todas</span></button>`),r.forEach(e=>{let n=i(e);a.append(`<button class="bl_cat_btn ${t===e?`active`:``}" data-cat="${e}" style="--cc:${n.color}"><i class="fas ${n.icon}"></i><span>${e}</span></button>`)})},x=(()=>{try{return JSON.parse(localStorage.getItem(p)||`[]`)}catch{return[]}})();if(x.length){let t=e(`#bl_cats`);x.forEach(e=>{let n=i(e);t.append(`<button class="bl_cat_btn" data-cat="${e}" style="--cc:${n.color}"><i class="fas ${n.icon}"></i><span>${e}</span></button>`)})}let S=t=>{let n=e(`#bl_grid`);t||(n.html(``),h=0);let r=t?h*d:0,i=g.slice(r,r+d);if(!i.length&&!t){e(`#bl_empty`).show(),e(`#bl_mas_wrap`).hide();return}e(`#bl_empty`).hide(),n.append(i.map(m).join(``)),o(`bl_fade`,n[0]),e(`#bl_mas_wrap`).toggle(r+d<g.length)},C=async(r=!1)=>{if(!v){v=!0,r||e(`#bl_grid`).html([,,,,,,].fill(l()).join(``)),e(`#bl_empty,#bl_mas_wrap`).hide(),e(`#bl_result_bar`).html(``);try{let n=await s(t,f,r);g=n.lista,_=g,(!x.length||r)&&b(g),e(`#bl_result_bar`).html(`<span><strong>${g.length}</strong> historia${g.length===1?``:`s`}</span>${u(n.fromCache)}`),S()}catch(t){console.error(`[blog]`,t),n(`Error`,`error`),e(`#bl_empty`).show()}v=!1}},w,T=t=>{clearTimeout(w),w=setTimeout(()=>{if(!t.trim()){g=_,S(),e(`#bl_result_bar`).html(`<span><strong>${g.length}</strong> historias</span>`);return}let n=t.toLowerCase();g=_.filter(e=>e.titulo?.toLowerCase().includes(n)||e.resumen?.toLowerCase().includes(n)||e.categoria?.toLowerCase().includes(n)||(e.tags||[]).some(e=>e.toLowerCase().includes(n))),e(`#bl_result_bar`).html(`<span><i class="fas fa-search"></i> <strong>${g.length}</strong> resultado${g.length===1?``:`s`} — "<em>${t}</em>"</span>`),S()},280)};await C(),e(document).on(`click.blog`,`.bl_cat_btn`,function(){let n=e(this).data(`cat`);n!==t&&(t=n,h=0,e(`.bl_cat_btn`).removeClass(`active`),e(this).addClass(`active`),C())}).on(`click.blog`,`.bl_ord_btn`,function(){let t=e(this).data(`ord`);t!==f&&(f=t,h=0,e(`.bl_ord_btn`).removeClass(`active`),e(this).addClass(`active`),C())}).on(`click.blog`,`#bl_refresh`,async function(){e(this).html(`<i class="fas fa-spinner fa-spin"></i>`).prop(`disabled`,!0),a(),localStorage.removeItem(p),await C(!0),e(this).html(`<i class="fas fa-rotate"></i>`).prop(`disabled`,!1),n(`Actualizado ✅`,`success`)}).on(`click.blog`,`#bl_search_toggle`,function(){let t=e(`#bl_search_bar`),n=t.is(`:visible`);t.stop(!0).slideToggle(180),n?(e(`#bl_search_inp`).val(``),g=_,S()):setTimeout(()=>e(`#bl_search_inp`).focus(),200)}).on(`click.blog`,`#bl_search_close`,()=>{e(`#bl_search_bar`).slideUp(160),e(`#bl_search_inp`).val(``),g=_,S()}).on(`input.blog`,`#bl_search_inp`,function(){T(e(this).val())}).on(`click.blog`,`#bl_mas`,()=>{h++,S(!0)}).on(`click.blog`,`.bl_card`,function(){let t=e(this).data(`slug`);t&&y(`/${t}`)}).on(`mouseenter.blog`,`.bl_card`,function(){c(e(this).data(`slug`))})},_=()=>e(document).off(`.blog`);export{_ as cleanup,g as init,h as render};