...
import{t as e}from"./vendor-BDh6mtVu.js";import{a as t,d as n,m as r,p as i,r as a,s as o,t as s,u as c}from"./widev-vc3w_AsM.js";import"./wii-DEYC667F.js";import{t as l}from"./preload-helper-TQeha_t2.js";import{E as u,_ as d,b as f,v as p,w as m,x as h,y as g}from"./firebase-BVO2Ew-x.js";import{n as _}from"./firebase-CSWtr52h.js";var v=()=>i(`wiSmile`),y=`gsTotalAlumnos`,b=`gsTotalClases`,x=`gsFeedReciente`,S=`gsMetricas`,C={feedSub:null},w=async()=>{let e=v();if(!e)return`
    <div class="gs_page">
  ...
  </div>`},T=async()=>{let t=v();if(!t)return;e(document).off(`.gs`);let n=i(`gsRealTime`)===!0;n&&e(`#gs_btn_rt`).addClass(`active`),await D(t),n&&A(t),e(document).on(`click.gs`,`#gs_btn_rt`,function(){let n=!e(this).hasClass(`active`);e(this).toggleClass(`active`,n),r(`gsRealTime`,n,24*365),n?A(t):(C.feedSub?.(),C.feedSub=null)}),e(document).on(`click.gs`,`#gs_refresh`,async function(){let n=e(this).find(`i`).addClass(`fa-spin`);[y,b,x,S].forEach(e=>localStorage.removeItem(e)),await D(t,!0),setTimeout(()=>n.removeClass(`fa-spin`),500)}),e(document).on(`click.gs`,`.nv_item`,function(t){t.preventDefault();let n=e(this).data(`page`);n&&l(async()=>{let{rutas:e}=await import(`./rutas-Duz3-mL_.js`).then(e=>e.r);return{rutas:e}},__vite__mapDeps([0])).then(({rutas:e})=>e.navigate(`/${n}`))}),e(document).on(`click.gs`,`.gs_fi_btn`,function(){r(`gsBuscarTerm`,e(this).data(`usuario`),1/60),l(async()=>{let{rutas:e}=await import(`./rutas-Duz3-mL_.js`).then(e=>e.r);return{rutas:e}},__vite__mapDeps([0])).then(({rutas:e})=>e.navigate(`/buscar`))})},E=()=>{C.feedSub?.(),e(document).off(`.gs`)};async function D(e,t=!1){await Promise.all([O(e,t),k(e,t)])}async function O(t,n=!1){if(!n){let t=i(y),n=i(b),r=i(S);if(t!=null&&e(`#gs_k_alumnos`).text(t),n!=null&&e(`#gs_k_clases`).text(n),r?.wpm&&e(`#gs_k_wpm`).text(r.wpm),r?.cert!=null&&e(`#gs_k_cert`).text(r.cert),t!=null&&n!=null&&r)return}try{let n=await d(h(u(_,`lecciones`),m(`gestor_id`,`==`,t.usuario)));n.empty&&(n=await d(h(u(_,`lecciones`),m(`gestorId`,`==`,t.usuario))));let i=n.docs.map(e=>e.data()),a=i.length,o=i.reduce((e,t)=>e+(t.wpmMax||0),0),s=a>0?Math.round(o/a):0,c=i.filter(e=>(e.completadas?.length||0)>=45&&(e.wpmMax||0)>=80).length;r(y,a,2),r(S,{wpm:s,cert:c},2),e(`#gs_k_alumnos`).text(a),e(`#gs_k_wpm`).text(s||`—`),e(`#gs_k_cert`).text(c);let l=await d(h(u(_,`clases`),m(`gestor_id`,`==`,t.usuario)));l.empty&&(l=await d(h(u(_,`clases`),m(`gestorId`,`==`,t.usuario)))),r(b,l.size,2),e(`#gs_k_clases`).text(l.size)}catch(e){console.error(`[gestor] Error KPIs`,e)}}async function k(e,t=!1){if(!(!t&&i(`gsRealTime`)===!0)){if(!t){let e=i(x);if(e?.length){j(e);return}}try{let t=await d(h(u(_,`lecciones`),m(`gestor_id`,`==`,e.usuario),f(`ultPractica`,`desc`),p(15)));t.empty&&(t=await d(h(u(_,`lecciones`),m(`gestorId`,`==`,e.usuario),f(`ultPractica`,`desc`),p(15))));let n=t.docs.map(e=>({usuario:e.id,...e.data()}));r(x,n,1/12),j(n)}catch{try{j((await d(h(u(_,`lecciones`),m(`gestor_id`,`==`,e.usuario),p(15)))).docs.map(e=>({usuario:e.id,...e.data()})))}catch{j([])}}}}function A(e){C.feedSub?.(),C.feedSub=g(h(u(_,`lecciones`),m(`gestor_id`,`==`,e.usuario),p(15)),e=>{let t=e.docs.map(e=>({usuario:e.id,...e.data()}));t.sort((e,t)=>{let n=e.ultPractica?.toDate?e.ultPractica.toDate().getTime():0;return(t.ultPractica?.toDate?t.ultPractica.toDate().getTime():0)-n}),j(t)})}function j(t){if(e(`#gs_feed_num`).text(t.length>0?`${t.length} Registros`:`0 Registros`),!t.length){e(`#gs_feed`).html(`
      <div class="gs_feed_empty">
