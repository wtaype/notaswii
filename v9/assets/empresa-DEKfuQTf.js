...
import{t as e}from"./vendor-BDh6mtVu.js";import{a as t,d as n,m as r,p as i,r as a,s as o,t as s,u as c}from"./widev-vc3w_AsM.js";import"./wii-DEYC667F.js";import{t as l}from"./preload-helper-TQeha_t2.js";import{E as u,_ as d,b as f,v as p,w as m,x as h}from"./firebase-BVO2Ew-x.js";import{n as g}from"./firebase-CSWtr52h.js";var _=()=>i(`wiSmile`),v=`epTotalEmpleados`,y=`epTotalEquipos`,b=`epMetricas`,x=`epFeed`,S=async()=>{let e=_();if(!e)return`<div class="epd_page"><div class="epd_empty"><i class="fas fa-lock"></i><p>Sin sesión activa.</p></div></div>`;let n=a(e.nombres||e.nombre||`Administrador`),r=o(e.nombres||e.nombre||``),l=e.foto||null,u=s(e.empresa||`NotasWii`),d=i(v)??`—`,f=i(y)??`—`,p=i(b)||{wpm:`—`,cert:`—`};return`
  <div class="epd_page">
    ...
    <div class="epd_hero">
      <div class="epd_hero_left">
        ...
        <div class="epd_hero_txt">
          ...
          <h1 class="epd_nombre">${n.split(` `)[0]}</h1>
          <div class="epd_tags">
      ...
      <div class="epd_hero_right">
        <div class="epd_date"><i class="fas fa-calendar-alt"></i> ${c()}</div>
      </div>
    </div>
    ...
    <div class="epd_feed_wrap">
  ...
  </div>`},C=async()=>{let t=_();t&&(e(document).off(`.epd`),await T(t),e(document).on(`click.epd`,`.nv_item`,function(t){t.preventDefault();let n=e(this).data(`page`);n&&l(async()=>{let{rutas:e}=await import(`./rutas-Duz3-mL_.js`).then(e=>e.r);return{rutas:e}},__vite__mapDeps([0])).then(({rutas:e})=>e.navigate(`/${n}`))}),e(document).on(`click.epd`,`#epd_btn_sync`,async function(){let n=e(this).find(`i`).addClass(`fa-spin`);[v,y,b,x].forEach(e=>localStorage.removeItem(e)),await T(t,!0),setTimeout(()=>n.removeClass(`fa-spin`),500)}))},w=()=>{e(document).off(`.epd`)};async function T(e,t=!1){await Promise.all([E(e,t),D(e,t)])}async function E(t,n){if(!(!n&&i(v)!=null))try{let n=await d(h(u(g,`lecciones`),m(`empresa_id`,`==`,t.usuario)));n.empty&&(n=await d(h(u(g,`lecciones`),m(`gestor_id`,`==`,t.usuario))));let i=n.docs.map(e=>e.data()),a=i.length,o=i.map(e=>e.wpmMax||0),s=a>0?Math.round(o.reduce((e,t)=>e+t,0)/a):0,c=i.filter(e=>(e.completadas?.length||0)>=45&&(e.wpmMax||0)>=80).length,l=await d(h(u(g,`clases`),m(`empresa_id`,`==`,t.usuario)));l.empty&&(l=await d(h(u(g,`clases`),m(`gestor_id`,`==`,t.usuario))));let f=l.size;r(v,a,2),r(y,f,2),r(b,{wpm:s,cert:c},2),e(`#epd_k_emp`).text(a),e(`#epd_k_eqp`).text(f),e(`#epd_k_wpm`).text(s||`—`),e(`#epd_k_cert`).text(c)}catch(e){console.error(`[empresa] KPI Error`,e)}}async function D(e,t){if(!t){let e=i(x);if(e?.length){O(e);return}}try{let t=await d(h(u(g,`lecciones`),m(`empresa_id`,`==`,e.usuario),f(`ultPractica`,`desc`),p(10)));t.empty&&(t=await d(h(u(g,`lecciones`),m(`gestor_id`,`==`,e.usuario),f(`ultPractica`,`desc`),p(10))));let n=t.docs.map(e=>({usuario:e.id,...e.data()}));r(x,n,1/12),O(n)}catch{try{let t=await d(h(u(g,`lecciones`),m(`empresa_id`,`==`,e.usuario),p(10)));t.empty&&(t=await d(h(u(g,`lecciones`),m(`gestor_id`,`==`,e.usuario),p(10)))),O(t.docs.map(e=>({usuario:e.id,...e.data()})))}catch{O([])}}}function O(t){if(!t.length){e(`#epd_feed_body`).html(`
      <div class="epd_empty">
        <i class="fas fa-ghost"></i>
        <p>No hay actividad registrada aún.</p>
      </div>`);return}let r=t.map(e=>{let t=e.nombre||e.usuario||`—`;return`
      <div class="epd_feed_item">
        ...
        <div class="epd_fi_time">${e.ultPractica?.toDate?n(e.ultPractica):`Reciente`}</div>
      </div>`}).join(``);e(`#epd_feed_body`).html(`<div class="epd_feed_list">${r}</div>`)}export{w as cleanup,C as init,S as render};