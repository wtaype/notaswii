import{t as e}from"./vendor-BDh6mtVu.js";import{i as t,p as n,s as r}from"./widev-vc3w_AsM.js";import{C as i,D as a,E as o,_ as s,g as c,w as l,x as u}from"./firebase-BVO2Ew-x.js";import{n as d}from"./firebase-CSWtr52h.js";var f=()=>n(`wiSmile`),p=[],m=()=>{let e=f();return!e||e.rol!==`admin`?`<div class="adp_page"><div class="adp_empty"><i class="fas fa-ban"></i><p>Acceso denegado.</p></div></div>`:`
  <div class="adp_page">
  ...
  </div>`},h=async()=>{let r=f();!r||r.rol!==`admin`||(e(document).off(`.adp`),await v(),e(document).on(`click.adp`,`#adp_btn_search`,async function(){let n=e(`#adp_inp_search`).val().trim();if(!n)return;let r=e(this).prop(`disabled`,!0).html(`<i class="fas fa-spinner fa-spin"></i>`);try{let e=await c(a(d,`smiles`,n));e.exists()?b({id:e.id,...e.data()}):t(`Usuario no encontrado`,`warning`)}catch{t(`Error en la búsqueda`,`error`)}r.prop(`disabled`,!1).html(`<i class="fas fa-search"></i> Buscar`)}),e(document).on(`keydown.adp`,`#adp_inp_search`,t=>{t.key===`Enter`&&e(`#adp_btn_search`).click()}),e(document).on(`click.adp`,`.adp_btn_edit`,function(){let t=e(this).data(`id`),n=p.find(e=>e.id===t);n&&b(n)}),e(document).on(`click.adp`,`.adp_role_opt`,function(){e(`.adp_role_opt`).removeClass(`selected`),e(this).addClass(`selected`)}),e(document).on(`click.adp`,`#adp_btn_save_rol`,async function(){let o=e(this).data(`id`),s=e(`.adp_role_opt.selected`).data(`rol`);if(!s||o===r.usuario&&s!==`admin`&&!confirm(`¿Estás seguro de quitarte los permisos de admin a ti mismo? Cerrarás tu propia sesión.`))return;let c=e(this).prop(`disabled`,!0).html(`<i class="fas fa-spinner fa-spin"></i> Guardando...`);try{if(await i(a(d,`smiles`,o),{rol:s}),t(`Rol actualizado a ${s.toUpperCase()}`,`success`),_(),e(`#adp_inp_search`).val(``),await v(),o===r.usuario&&s!==`admin`){let e=n(`wiSmile`);e.rol=s,localStorage.setItem(`wiSmile`,JSON.stringify(e)),setTimeout(()=>location.reload(),1e3)}}catch{t(`Error al guardar`,`error`),c.prop(`disabled`,!1).html(`Guardar Permisos`)}}),e(document).on(`click.adp`,`.adp_modal_close, .adp_btn_cancel`,_),e(document).on(`click.adp`,`.adp_modal_bg`,t=>{e(t.target).hasClass(`adp_modal_bg`)&&_()}))},g=()=>{e(document).off(`.adp`)};function _(){e(`#adp_modales`).html(``)}async function v(){try{p=(await s(u(o(d,`smiles`),l(`rol`,`in`,[`gestor`,`empresa`,`admin`])))).docs.map(e=>({id:e.id,...e.data()})),y()}catch(t){console.error(`[admin_permisos] Error VIPs:`,t),e(`#adp_grid`).html(`<div class="adp_empty"><i class="fas fa-exclamation-triangle"></i><p>Error cargando cuentas VIP.</p></div>`)}}function y(){if(!p.length){e(`#adp_grid`).html(`<div class="adp_empty"><i class="fas fa-user-shield"></i><p>No hay cuentas privilegiadas asignadas aún.</p></div>`);return}let t={gestor:`#8b5cf6`,empresa:`#f59e0b`,admin:`#020617`},n=p.map(e=>{let n=e.rol||`smile`,i=t[n]||`#38bdf8`,a=e.nombres||e.nombre||e.id,o=e.email||`Sin correo registrado`;return`
