import{t as e}from"./vendor-BDh6mtVu.js";import{S as t,g as n,h as r,i}from"./widev-vc3w_AsM.js";import{t as a}from"./wii-DEYC667F.js";import{t as o}from"./preload-helper-TQeha_t2.js";var s=`misNotas`,c=20,l=()=>`min`+Date.now(),u={idea:{icon:`fa-lightbulb`,label:`Idea`,color:`#0EBEFF`},nota:{icon:`fa-file-lines`,label:`Nota`,color:`#29C72E`},tarea:{icon:`fa-check-double`,label:`Tarea`,color:`#7000FF`},link:{icon:`fa-link`,label:`Link`,color:`#FFB800`}},d=()=>{try{return JSON.parse(localStorage.getItem(s)||`[]`).map(e=>({...e,link:e.link||e.url||``,pin:e.pin??e.fijada??!1,creado:e.creado||e.fecha||Date.now()}))}catch{return[]}},f=e=>localStorage.setItem(s,JSON.stringify(e)),p=(e,t)=>`${u[e]?.label||`Nota`} ${t.filter(t=>t.tipo===e).length+1}`,m=null,h=async()=>{if(m)return m;try{let{db:e}=await o(async()=>{let{db:e}=await import(`./firebase-CSWtr52h.js`).then(e=>e.r);return{db:e}},[]);return m=e,e}catch{return null}},g=async e=>{let t=n.user;if(!t?.usuario)return;let r=await h();if(r)try{let{doc:n,setDoc:i,serverTimestamp:a}=await o(async()=>{let{doc:e,setDoc:t,serverTimestamp:n}=await import(`./firebase-BVO2Ew-x.js`).then(e=>e.f);return{doc:e,setDoc:t,serverTimestamp:n}},[]),s=!e.synced;s&&(e.synced=!0),await i(n(r,`misnotas`,e.id),{usuario:String(t.usuario||``),email:String(t.email||``),titulo:String(e.titulo||``),contenido:String(e.contenido||``),link:String(e.link||``),pin:!!e.pin,tipo:e.tipo||`idea`,...s?{creado:a()}:{actualizado:a()}},{merge:!0})}catch(e){console.error(`[misnotas] guardarNube:`,e)}},_=async e=>{if(!n.user?.usuario)return;let t=await h();if(t)try{let{doc:n,deleteDoc:r}=await o(async()=>{let{doc:e,deleteDoc:t}=await import(`./firebase-BVO2Ew-x.js`).then(e=>e.f);return{doc:e,deleteDoc:t}},[]);await r(n(t,`misnotas`,e))}catch{}},v=async()=>{let e=n.user;if(!e?.email)return null;let t=await h();if(!t)return null;try{let{collection:n,getDocs:r,query:i,where:a}=await o(async()=>{let{collection:e,getDocs:t,query:n,where:r}=await import(`./firebase-BVO2Ew-x.js`).then(e=>e.f);return{collection:e,getDocs:t,query:n,where:r}},[]);return(await r(i(n(t,`misnotas`),a(`email`,`==`,e.email)))).docs.map(e=>{let t=e.data();return{id:e.id,tipo:t.tipo||`idea`,titulo:t.titulo||``,contenido:t.contenido||``,link:String(t.link||``),pin:!!t.pin,creado:t.creado?.toMillis?.()||Date.now(),synced:!0}})}catch{return null}},y=[],b=`todas`,x=``,S=null,C=`idea`,w=e=>{let n=u[e.tipo]||u.idea,r=new Date(e.creado).toLocaleDateString(`es-PE`,{day:`2-digit`,month:`short`,year:`numeric`}),i=(e.contenido||``).slice(0,120);return`
  ...
  <div class="mn_header">
    ...
    <div class="mn_header_right">
      ...
      <div class="mn_filtros" id="mn_filtros">
        ...
        ${Object.entries(u).map(([e,n])=>`<button class="mn_filtro" data-tipo="${e}" style="--fc:${n.color}" ${t(n.label)}><i class="fas ${n.icon}"></i></button>`).join(``)}
      </div>
    </div>
  </div>
    ...
    <aside class="mn_panel" id="mn_panel">
      ...
      <div class="mn_form" id="mn_form">
        <div class="mn_tipo_sel" id="mn_tipo_sel">
          ${Object.entries(u).map(([e,n])=>`<button class="mn_tipo_btn ${e===`idea`?`active`:``}" data-tipo="${e}" style="--tc:${n.color}" ${t(n.label)}><i class="fas ${n.icon}"></i> ${n.label}</button>`).join(``)}
        </div>
