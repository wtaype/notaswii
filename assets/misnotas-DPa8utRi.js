import{t as e}from"./vendor-BDh6mtVu.js";import{S as t,f as n,h as r,i,m as a,p as o}from"./widev-CHrYgt0x.js";import{t as s}from"./wii-Ca7PnfLv.js";import{t as c}from"./preload-helper-D8R59StC.js";var l=`misNotas`,u=20,d=()=>`min`+Date.now(),f={idea:{icon:`fa-lightbulb`,label:`Idea`,color:`#0EBEFF`},nota:{icon:`fa-file-lines`,label:`Nota`,color:`#29C72E`},tarea:{icon:`fa-check-double`,label:`Tarea`,color:`#7000FF`},link:{icon:`fa-link`,label:`Link`,color:`#FFB800`}},p=[{id:`ej1`,tipo:`idea`,titulo:`¡Bienvenido a Mis Notas!`,contenido:`Guarda tus ideas rápidas. Dale clic al icono de la chincheta para fijarlas arriba.`,pin:!0,creado:Date.now()},{id:`ej2`,tipo:`tarea`,titulo:`Sincroniza en la nube`,contenido:`Crea una cuenta gratuita para guardar todo en la nube y acceder desde cualquier dispositivo.`,pin:!1,creado:Date.now()-1e3},{id:`ej3`,tipo:`link`,titulo:`Mis Proyectos`,link:`https://wtaype.web.app/proyectos`,pin:!1,creado:Date.now()-2e3}],m={get:()=>{let e=localStorage.getItem(l);return e===null&&!r.user?[...p]:(n(l)||(e?.startsWith(`[`)?JSON.parse(e):[])).map(e=>({...e,link:e.link||e.url||``,pin:e.pin??e.fijada??!1,creado:e.creado||e.fecha||Date.now()}))},set:e=>o(l,e,8760)},h=(e,t)=>`${f[e]?.label||`Nota`} ${t.filter(t=>t.tipo===e).length+1}`,g=null,_=async()=>{if(g)return g;try{let{db:e}=await c(async()=>{let{db:e}=await import(`./firebase-C44YYj9C.js`).then(e=>e.r);return{db:e}},[]);return g=e,e}catch{return null}},v=async e=>{let t=r.user;if(!t?.usuario)return;let n=await _();if(n)try{let{doc:r,setDoc:i,serverTimestamp:a}=await c(async()=>{let{doc:e,setDoc:t,serverTimestamp:n}=await import(`./firebase-CU10hIs1.js`).then(e=>e.f);return{doc:e,setDoc:t,serverTimestamp:n}},[]),o=!e.synced;o&&(e.synced=!0),await i(r(n,`misnotas`,e.id),{usuario:String(t.usuario||``),email:String(t.email||``),titulo:String(e.titulo||``),contenido:String(e.contenido||``),link:String(e.link||``),pin:!!e.pin,tipo:e.tipo||`idea`,...o?{creado:a()}:{actualizado:a()}},{merge:!0})}catch(e){console.error(`[misnotas] guardarNube:`,e)}},y=async e=>{if(!r.user?.usuario)return;let t=await _();if(t)try{let{doc:n,deleteDoc:r}=await c(async()=>{let{doc:e,deleteDoc:t}=await import(`./firebase-CU10hIs1.js`).then(e=>e.f);return{doc:e,deleteDoc:t}},[]);await r(n(t,`misnotas`,e))}catch{}},b=async()=>{let e=r.user;if(!e?.email)return null;let t=await _();if(!t)return null;try{let{collection:n,getDocs:r,query:i,where:a}=await c(async()=>{let{collection:e,getDocs:t,query:n,where:r}=await import(`./firebase-CU10hIs1.js`).then(e=>e.f);return{collection:e,getDocs:t,query:n,where:r}},[]);return(await r(i(n(t,`misnotas`),a(`email`,`==`,e.email)))).docs.map(e=>{let t=e.data();return{id:e.id,tipo:t.tipo||`idea`,titulo:t.titulo||``,contenido:t.contenido||``,link:String(t.link||``),pin:!!t.pin,creado:t.creado?.toMillis?.()||Date.now(),synced:!0}})}catch{return null}},x=[],S=`todas`,C=``,w=null,T=`idea`,E=e=>{let n=f[e.tipo]||f.idea,r=new Date(e.creado).toLocaleDateString(`es-PE`,{day:`2-digit`,month:`short`,year:`numeric`}),i=(e.contenido||``).slice(0,120);return`
  <article class="mn_card" data-id="${e.id}" data-tipo="${e.tipo}" style="--bc:${n.color}" title="Clic para editar">
    <div class="mn_card_bar"></div>
    <div class="mn_card_top">
      <span class="mn_badge"><i class="fas ${n.icon}"></i> ${n.label}</span>
      <div class="mn_card_acts">
        <button class="mn_act mn_act_copy" data-id="${e.id}" ${t(`Copiar`)}><i class="fas fa-copy"></i></button>
        <button class="mn_act mn_act_del"  data-id="${e.id}" ${t(`Eliminar`,``,`error`)}><i class="fas fa-trash"></i></button>
      </div>
    </div>
    ...
    ${i?`<p class="mn_card_body">${i}${(e.contenido||``).length>120?`…`:``}</p>`:``}
    <div class="mn_card_foot">
      <span class="mn_fecha"><i class="fas fa-clock"></i> ${r}</span>
  ...
  <div class="mn_header">
    ...
    <div class="mn_header_right">
      ...
      <div class="mn_filtros" id="mn_filtros">
        <button class="mn_filtro active" data-tipo="todas" ${t(`Todas`)}><i class="fas fa-layer-group"></i></button>
        ${Object.entries(f).map(([e,n])=>`<button class="mn_filtro" data-tipo="${e}" style="--fc:${n.color}" ${t(n.label)}><i class="fas ${n.icon}"></i></button>`).join(``)}
      </div>
    </div>
  </div>
  ...
  <div class="mn_layout">
    <aside class="mn_panel" id="mn_panel">
      <div class="mn_panel_head">
        ...
        <button class="mn_panel_new" id="mn_panel_new" ${t(`Nueva nota`)}>Nuevo <i class="fas fa-plus"></i></button>
      </div>
      <div class="mn_form" id="mn_form">
        <div class="mn_tipo_sel" id="mn_tipo_sel">
          ${Object.entries(f).map(([e,n])=>`<button class="mn_tipo_btn ${e===`idea`?`active`:``}" data-tipo="${e}" style="--tc:${n.color}" ${t(n.label)}><i class="fas ${n.icon}"></i> ${n.label}</button>`).join(``)}
        </div>
