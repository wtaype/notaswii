import{b as e,f as t,i as n,p as r}from"./widev-H4r-I7mR.js";import{A as i,C as a,D as o,E as s,S as c,g as l,m as u,w as d,x as f,y as p}from"./firebase-CCUuWaKQ.js";import{n as m}from"./firebase-DG8WAU6u.js";var h=null,g=t(`misProyectosLinkwii`)||[],_=g.length>0?g[0].slug:null,v=[{hex:`#FFFFFF`,name:`auto`},{hex:`#FFDA34`,name:`Oro`},{hex:`#3cd741`,name:`Success`},{hex:`#ffa726`,name:`Warning`},{hex:`#00a8e6`,name:`Info`},{hex:`#0EBEFF`,name:`Cielo`},{hex:`#FF5C69`,name:`Dulce`},{hex:`#29C72E`,name:`Paz`},{hex:`#7000FF`,name:`Mora`},{hex:`#21273B`,name:`Futuro`},{hex:`#dddddd`,name:`Offline`}],y=[``,`fas fa-link`,`fab fa-instagram`,`fab fa-tiktok`,`fab fa-youtube`,`fab fa-whatsapp`,`fab fa-facebook`,`fab fa-spotify`,`fab fa-x-twitter`,`fab fa-linkedin`,`fab fa-telegram`,`fas fa-globe`,`fas fa-store`,`fas fa-envelope`,`fas fa-phone`],b=e=>e?e.includes(`instagram.com`)?`fab fa-instagram`:e.includes(`tiktok.com`)?`fab fa-tiktok`:e.includes(`youtube.com`)||e.includes(`youtu.be`)?`fab fa-youtube`:e.includes(`whatsapp.com`)||e.includes(`wa.me`)?`fab fa-whatsapp`:e.includes(`facebook.com`)||e.includes(`fb.me`)?`fab fa-facebook`:e.includes(`spotify.com`)?`fab fa-spotify`:e.includes(`twitter.com`)||e.includes(`x.com`)?`fab fa-x-twitter`:e.includes(`linkedin.com`)?`fab fa-linkedin`:e.includes(`t.me`)||e.includes(`telegram`)?`fab fa-telegram`:e.includes(`pinterest.com`)?`fab fa-pinterest`:`fas fa-link`:``,x=async()=>t(`wiSmile`)?`
  ...
  `,document.getElementById(`btn_update_info`)?.addEventListener(`click`,async t=>{let s=t.currentTarget;e(s,!0,`Guardando`);try{await a(o(m,`linkwiis`,r.slug),{desc:document.getElementById(`edit_desc`).value,logo:document.getElementById(`edit_logo`).value,color:document.getElementById(`edit_color`).value,actualizado:i()}),n(`Apariencia guardada ✨`,`success`)}catch{n(`Error al guardar`)}e(s,!1,`<i class="fas fa-save"></i> Guardar`)}),document.getElementById(`btn_add_link_arr`)?.addEventListener(`click`,async()=>{let e=[...r.links||[],{titulo:``,url:``,icon:``}];await a(o(m,`linkwiis`,r.slug),{links:e})}),document.querySelectorAll(`.btn_rm_link`).forEach(e=>{e.addEventListener(`click`,async e=>{if(!confirm(`¿Estás seguro de eliminar este enlace? Esta acción no se puede deshacer.`))return;let t=parseInt(e.currentTarget.dataset.index),i=[...r.links||[]];i.splice(t,1);try{await a(o(m,`linkwiis`,r.slug),{links:i}),n(`Enlace eliminado ✨`,`success`)}catch{n(`Error al eliminar`)}})}),document.getElementById(`btn_save_links`)?.addEventListener(`click`,async t=>{let s=t.currentTarget;e(s,!0,`Guardando`);let c=r.links.filter(e=>e.titulo.trim()||e.url.trim());c.push({titulo:``,url:``,icon:``});try{await a(o(m,`linkwiis`,r.slug),{links:c,actualizado:i()}),n(`Enlaces guardados ✨`,`success`)}catch{n(`Error al guardar enlaces`)}e(s,!1,`<i class="fas fa-save"></i> Guardar`)}),document.getElementById(`btn_del_main`)?.addEventListener(`click`,async()=>{if(confirm(`¿Eliminar definitivamente el proyecto /${r.slug}? Esta acción no se puede deshacer.`))try{await u(o(m,`linkwiis`,r.slug)),_=null,n(`Proyecto eliminado ✨`,`success`)}catch{n(`Error al eliminar`)}})},w={instagram:`linear-gradient(45deg, #f58529, #dd2a7b, #8134af, #515bd4)`,tiktok:`#000`,youtube:`#ff0000`,whatsapp:`#25D366`,facebook:`#1877F2`,twitter:`#000`,linkedin:`#0077b5`,telegram:`#0088cc`,onlyfans:`#00aff0`,pinterest:`#E60023`,web:`#00a8e6`,link:`#FFDA34`},T=e=>e?e.includes(`instagram.com`)?`instagram`:e.includes(`tiktok.com`)?`tiktok`:e.includes(`youtube.com`)||e.includes(`youtu.be`)?`youtube`:/whatsapp|wa\.me/.test(e)?`whatsapp`:e.includes(`facebook.com`)||e.includes(`fb.me`)?`facebook`:/twitter\.com|x\.com/.test(e)?`twitter`:e.includes(`linkedin.com`)?`linkedin`:/t\.me|telegram/.test(e)?`telegram`:e.includes(`onlyfans.com`)?`onlyfans`:e.includes(`pinterest.com`)?`pinterest`:/\.(com|net|org|co|info|es|app|io|me)/.test(e)?`web`:`link`:`link`,E=()=>{let e=document.getElementById(`cr_phone_render`);if(!e)return;let t=g.find(e=>e.slug===_);if(!t){e.innerHTML=`<div class="ad_empty" style="margin-top:40%; font-size:0.75rem; color:var(--tx3);"><i class="fas fa-mobile-alt" style="font-size:2rem; display:block; margin-bottom:1vh; opacity:0.3;"></i>En vivo</div>`;return}let n=t.color||`#FFFFFF`,r=(t.links||[]).filter(e=>e.titulo).map(e=>{let t=e.icon||b(e.url),r=n;(!r||r.toLowerCase()===`#ffffff`)&&(r=w[T(e.url)]);let i=r&&r.includes(`gradient`);return`
  ...
  `};window.crCicloIcono=e=>{let t=g.find(e=>e.slug===_);if(!t)return;let n=t.links[e],r=n.icon||``,i=y.indexOf(r);i===-1&&(i=0);let a=y[(i+1)%y.length];n.icon=a;let o=document.getElementById(`icon_prev_${e}`);o&&(o.className=a||b(n.url)||`fas fa-link`),E()},window.crPreviewEnlace=(e,t)=>{let n=parseInt(e.dataset.idx),r=g.find(e=>e.slug===_);if(!r)return;let i=e.value;if(t===`url`&&i&&!i.startsWith(`http`)&&(i=`https://`+i),r.links[n][t]=i,t===`url`&&!r.links[n].icon){let e=document.getElementById(`icon_prev_${n}`);e&&(e.className=b(i)||`fas fa-link`)}E()},window.crPreviewApariencia=(e,t)=>{let n=g.find(e=>e.slug===_);n&&(n[t]=e.value,E())},window.crSeleccionar=e=>{_=e,S(),C(),E()},window.crCopiar=e=>{navigator.clipboard.writeText(`${window.location.origin}/${e}`).then(()=>n(`Enlace copiado 📋`,`success`))},window.crElegirColor=e=>{document.querySelectorAll(`.cr_swatch`).forEach(e=>e.classList.remove(`selected`)),e.classList.add(`selected`);let t=e.dataset.color;document.getElementById(`edit_color`).value=t;let n=g.find(e=>e.slug===_);n&&(n.color=t),E()};var D=()=>{let e=t(`wiSmile`);if(!e)return;let a=document.getElementById(`btn_nuevo_proy`),u=document.getElementById(`form_nuevo_proy`),v=document.getElementById(`inp_slug`);a?.addEventListener(`click`,()=>{u.style.display=`block`,v.focus()}),document.getElementById(`btn_cancel_proy`)?.addEventListener(`click`,()=>{u.style.display=`none`,v.value=``}),document.getElementById(`btn_save_proy`)?.addEventListener(`click`,async t=>{let r=v.value.trim().toLowerCase().replace(/[^a-z0-9-]/g,``);if(!r)return n(`Ingresa un slug válido`);let a=t.currentTarget,s=a.innerHTML;a.innerHTML=`<i class="fas fa-spinner fa-spin"></i>`,a.disabled=!0;try{let t=o(m,`linkwiis`,r);if((await l(t)).exists())return n(`Ese slug ya está ocupado. Elige otro.`);await c(t,{slug:r,usuario:e.usuario,email:e.email,estado:!0,vistas:0,desc:`¡Bienvenido a mi Linkwii!`,logo:e.avatar||``,color:`#FFFFFF`,links:[],creado:i(),actualizado:i()}),n(`Proyecto creado 🎉`,`success`),u.style.display=`none`,v.value=``,window.crSeleccionar(r)}catch(e){console.error(e),n(`Error al crear`)}finally{a.innerHTML=s,a.disabled=!1}});let y=f(s(m,`linkwiis`),d(`usuario`,`==`,e.usuario));g.length>0&&(S(),C(),E()),h=p(y,e=>{g=e.docs.map(e=>({slug:e.id,...e.data()})),r(`misProyectosLinkwii`,g),!_&&g.length>0?_=g[0].slug:_&&!g.find(e=>e.slug===_)&&(_=g.length>0?g[0].slug:null),S(),C(),E()})},O=()=>{h&&h(),delete window.crSeleccionar,delete window.crCopiar,delete window.crElegirColor};export{O as cleanup,D as init,x as render};