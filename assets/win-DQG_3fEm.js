import{t as e}from"./vendor-BDh6mtVu.js";import{S as t,i as n,p as r,x as i}from"./widev-BLqPD_AU.js";import"./rutas-DKyxVXgu.js";import{A as a,D as o,E as s,T as c,_ as l,m as u,v as d,w as f,x as p}from"./firebase-CCUuWaKQ.js";import{n as m}from"./firebase-BNeWaP4M.js";var h=[],g=null,_=``,v=!0,y=!1,b=null,x=`wiWin`,S=`wi_win_cache`,C=()=>r(`wiSmile`)||{},w=e=>localStorage.setItem(S,JSON.stringify(e)),T=()=>JSON.parse(localStorage.getItem(S)||`[]`),E=()=>h.sort((e,t)=>!!t.pin-+!!e.pin||(t.fechaActualizado?.seconds||0)-(e.fechaActualizado?.seconds||0)),D=()=>({seconds:Math.floor(Date.now()/1e3)}),O=async(t,n=!1)=>{if(y=!t?.email,y){v=!1,k();return}n||e(`.es_btn_refresh`).addClass(`syncing`);try{h=(await l(p(s(m,x),f(`email`,`==`,t.email),d(100)))).docs.map(e=>({_fsId:e.id,...e.data()})),E(),w(h),v=!1,k()}catch{v=!1,n||L()}finally{e(`.es_btn_refresh`).removeClass(`syncing`)}},k=()=>{g||=h.find(e=>e.pin)||h[0]||null,L()},A=async(t=!1)=>{if(!g)return;let r=C(),s=e(`#btnS2`),l=e(`.es_in_title_h`).val().trim()||`Untitled`,u=e(`.es_editor`).html();g.titulo=l,g.contenido=u,g._dirty&&(g.fechaActualizado=D());let d=h.filter(e=>e._dirty);if(!d.length){t&&n(`Sin cambios por guardar`,`info`,800);return}if(w(h),t&&i(s,!0,`Guardando`),y){d.forEach(e=>e._dirty=!1),w(h),F(),t&&setTimeout(()=>{i(s,!1,`Guardado`),setTimeout(()=>s.html(`<i class="fas fa-save"></i> <span id="iconSync">Guardar</span>`),1500)},600);return}try{let e=c(m);for(let t of d){let n={id:t.id,titulo:t.titulo||`Untitled`,contenido:t.contenido||``,email:r.email,usuario:r.usuario||`Public`,fecha:t.fecha||a(),fechaActualizado:a(),pin:!!t.pin};e.set(o(m,x,t._fsId),n),t._dirty=!1}await e.commit(),w(h),F(),t&&(n(`Sincronización Exitosa ✨`,`success`,800),i(s,!1,`Guardado`))}catch(e){t&&(console.error(`Save Error:`,e),n(`Error al guardar`,`error`),i(s,!1,`Reintentar`))}finally{t&&setTimeout(()=>{e(`#btnS2`).length&&e(`#btnS2`).html(`<i class="fas fa-save"></i> <span>Guardar</span>`)},2e3)}},j=async()=>{let t=C(),n=`win${Date.now()}`,r=D(),i={_fsId:n,id:n,titulo:``,contenido:``,pin:!1,email:t.email||`guest`,usuario:t.usuario||`Public`,fecha:r,fechaActualizado:r,_dirty:!0};h.unshift(i),g=i,w(h),L(),e(`.es_in_title_h`).focus(),e(`.es_container`).removeClass(`menu-open`)},M=async(t,r=null)=>{if(confirm(`¿Eliminar?`)){r&&i(e(r),!0,`...`);try{h=h.filter(e=>e._fsId!==t),g?._fsId===t&&(g=h[0]||null),w(h),y||await u(o(m,x,t)),L()}catch{r&&i(e(r),!1,`<i class="fas fa-trash-alt"></i>`),n(`Error al eliminar`,`error`)}}},N=async e=>{let t=h.find(t=>t._fsId===e);t&&(t.pin=!t.pin,t._dirty=!0,t.fechaActualizado=D(),E(),w(h),L())},P=()=>{e(`.es_tool_btn[data-cmd]`).each(function(){let t=e(this).data(`cmd`);try{document.queryCommandState(t)?e(this).addClass(`active`):e(this).removeClass(`active`)}catch{}});try{let t=window.getSelection().anchorNode;if(t){let n=t.nodeType===3?t.parentNode:t;if(e(n).closest(`.es_editor`).length){let t=window.getComputedStyle(n);if(t.fontSize&&e(`#winFontSize`).val(parseInt(t.fontSize)),t.fontFamily){let n=t.fontFamily.split(`,`)[0].replace(/['"]/g,``);e(`#winFontFamily option`).each(function(){(e(this).text()===n||e(this).val().includes(n))&&e(`#winFontFamily`).val(e(this).val())})}let r=e(n).closest(`p, div, h1, h2, h3, h4, h5, h6, li`);if(r.length){let n=r[0].style.lineHeight;n&&e(`#winLineHeight`).val(n),r[0].style.marginBottom===`0px`||t.marginBottom===`0px`?e(`#btnNoMargin`).addClass(`active`):e(`#btnNoMargin`).removeClass(`active`)}}}}catch{}},F=()=>{let n=h.filter(e=>(e.titulo||``).toLowerCase().includes(_.toLowerCase()));e(`.es_list_items_final`).html(n.map(n=>{let r=n.titulo||`Untitled`,i=e(`<div>`).html(n.contenido||``).text().trim()||`Sin contenido...`,a=i.length>30?i.substring(0,30)+`...`:i,o=!n._dirty,s=new Date((n.fechaActualizado?.seconds||Date.now()/1e3)*1e3).toLocaleDateString(`es-ES`,{day:`2-digit`,month:`short`});return`
        <div class="es_item_final ${g?._fsId===n._fsId?`active`:``}" data-id="${n._fsId}">
            <div class="it_r1">
                <strong class="it_title">${n.pin?`<i class="fas fa-thumbtack pin_ico"></i> `:``}${r}</strong>
                <div class="it_icons">
                    ...
                    <i class="fas fa-thumbtack it_action btnPin" data-id="${n._fsId}" style="${n.pin?`color:var(--mco)`:``}" ${t(n.pin?`Desanclar`:`Anclar`)}></i>
                    <i class="fas fa-trash-alt it_action btnDel" data-id="${n._fsId}" ${t(`Borrar`)}></i>
                </div>
            </div>
            <div class="it_r2">
        ...
        </div>`}).join(``)||`<div class="txc" style="margin-top:20px; opacity:0.4; font-size:12px;">No hay documentos</div>`)},I=()=>{let n=e(`.es_left`);if(n.length){if(v&&!h.length)return n.html(`<div class="es_skeleton"> <div class="sk_line" style="width:40%"></div> <div class="sk_line"></div> </div>`);if(!g)return n.html(`<div style="margin:auto; text-align:center;"><button class="es_btn_new_final" id="btnS1" style="width:280px">+ Nuevo Win</button></div>`);n.html(`
        <div class="es_page">
