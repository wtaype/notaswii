...
</div>`,w=null,T=async()=>{let r=h.get(),i=()=>e(`#lk_grid`).html(`<div class="lk_skeleton"></div>`.repeat(3)),c=t=>e(`[data-id="${t}"] .lk_act_cloud`).removeClass(`fa-cloud-arrow-up lk_cloud_pen`).addClass(`fa-cloud lk_cloud_ok`).attr(`data-witip`,`En nube`),l=()=>e(`#lk_count`).text(`${r.length} guardado${r.length===1?``:`s`}`),d=()=>[...r].sort((e,t)=>e.pin&&!t.pin?-1:!e.pin&&t.pin?1:(t.creado||0)-(e.creado||0)),m=async()=>{let e=d();await n(`#lk_grid`,e.length?e.map(_).join(``):`<div class="lk_empty"><i class="fas fa-satellite-dish"></i><span>No tienes enlaces guardados. Pega uno arriba para empezar.</span></div>`,80),e.length&&s([`.lk_grid > *`],60),l()},v=e=>r.find(t=>t.id===e),C=()=>{r=r.filter(e=>!e.id.startsWith(`ej`));let t=e(`#lk_in_url`),n=t.val().trim();if(!n)return;/^https?:\/\//i.test(n)||(n=`https://`+n);let i=g(n),u=i.charAt(0).toUpperCase()+i.slice(1),d={id:p(),titulo:u,contenido:n,pin:!1,creado:Date.now(),synced:!1};r.unshift(d),h.set(r),t.val(``),e(`.lk_empty`).remove();let f=e(_(d)).css(`opacity`,0);e(`#lk_grid`).prepend(f),s([`#lk_${d.id}`],0),l(),o(`Enlace guardado`,`success`),a.user&&y(d).then(()=>{d.synced=!0,h.set(r),c(d.id)})};e(document).on(`click`,`#lk_btn_add`,C).on(`keydown`,`#lk_in_url`,function(e){e.key===`Enter`&&(e.preventDefault(),C())}).on(`click`,`#lk_btn_refresh`,async function(){let t=e(this).find(`i`);if(t.hasClass(`lk_spin`))return;t.addClass(`lk_spin`);let n=await S();n&&(JSON.stringify(n)!==JSON.stringify(r)&&(r=n,h.set(r),m()),o(`Sincronizado ✓`,`success`)),t.removeClass(`lk_spin`)}).on(`change`,`.lk_tit`,function(){let t=e(this).closest(`.lk_card`).data(`id`),n=v(t);if(!n)return;n.titulo=e(this).val();let i=n.titulo?n.titulo.charAt(0).toUpperCase():`L`;e(this).closest(`.lk_card`).find(`.lk_icon`).text(i),h.set(r),a.user&&b(n).then(()=>c(t))}).on(`click`,`.lk_act_pin`,function(){let t=e(this).data(`id`),n=v(t);n&&(n.pin=!n.pin,h.set(r),m(),o(n.pin?`Enlace fijado ✓`:`Desanclado`,`success`),a.user&&b(n).then(()=>c(t)))}).on(`click`,`.lk_act_del`,function(){let t=e(this).data(`id`);confirm(`¿Seguro que deseas eliminar este enlace?`)&&(r=r.filter(e=>e.id!==t),h.set(r),e(`#lk_${t}`).css(`overflow`,`hidden`).slideUp(280,function(){e(this).remove(),r.length||m()}),l(),o(`Enlace eliminado`,`success`),a.user&&x(t))}).on(`click`,`.lk_act_copy`,function(){let n=v(e(this).data(`id`));n&&t(n.contenido||n.url,this,`¡URL copiada!`)}),s([`.lk_hero_left`,`.lk_add_box`],50),setTimeout(()=>e(`#lk_in_url`).focus(),150),m(),w=a.on(async t=>{if(e(`#lk_btn_refresh`).toggle(!!t),t){r.length===0&&i();let e=await S();e&&JSON.stringify(e)!==JSON.stringify(r)&&(r=e,h.set(r),m())}else localStorage.removeItem(f),r=h.get(),m()}),console.log(`✅ ${u} v13 · Links OK`)},E=()=>{e(document).off(`click keydown change`,`#lk_btn_add, #lk_in_url, #lk_btn_refresh, .lk_tit, .lk_act_pin, .lk_act_del, .lk_act_copy`),w?.()};export{E as cleanup,T as init,C as render};