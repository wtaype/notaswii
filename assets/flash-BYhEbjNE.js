  ...
  <div class="fl_timeline">
    ...
    <div id="fl_grid" class="fl_grid">
      <div class="fl_skeleton"></div><div class="fl_skeleton"></div><div class="fl_skeleton"></div>
...
</div>`,S=null,C=async()=>{let r=e(`#fl_input`),s=p.get(),l=()=>e(`#fl_grid`).html(`<div class="fl_skeleton"></div>`.repeat(3)),f=t=>e(`[data-id="${t}"] .fl_act_cloud`).removeClass(`fl_act fl_act_cloud`).addClass(`fl_act fl_act_cloud synced`).attr(`title`,`Guardado en nube`).html(`<i class="fas fa-cloud"></i>`),m=()=>[...s].sort((e,t)=>e.pin&&!t.pin?-1:!e.pin&&t.pin?1:(t.creado||0)-(e.creado||0)).slice(0,50),g=async()=>{let e=m();await n(`#fl_grid`,e.length?e.map(h).join(``):`<div class="fl_empty">Aún no tienes destellos de genialidad hoy...</div>`,80),e.length&&o([`.fl_grid > *`],50)},x=async(t=!1)=>{if(!i.user)return;t&&l();let n=e(`#fl_btn_refresh i`).addClass(`fl_spin`);try{let e=await b();e&&(s=e,p.set(s))}finally{n.removeClass(`fl_spin`),g()}},C=()=>{let t=r.val().trim();if(!t)return;s=s.filter(e=>!e.id.startsWith(`ej`));let n={id:d(),contenido:t,pin:!1,creado:Date.now()};s.unshift(n),p.set(s),r.val(``).css(`height`,`auto`),e(`.fl_input_box`).removeClass(`active`),a(`¡Flash capturado! ⚡`,`success`),g(),i.user&&_(n).then(()=>{n.synced=!0,p.set(s),f(n.id)})},w=e=>s.find(t=>t.id===e);r.on(`input`,function(){this.style.height=`auto`,this.style.height=`${this.scrollHeight}px`,e(`.fl_input_box`).toggleClass(`active`,!!this.value.trim())}),e(document).on(`click`,`#fl_btn_send`,C).on(`click`,`#fl_btn_refresh`,()=>x(!0)).on(`keydown`,`#fl_input`,e=>{e.key===`Enter`&&!e.shiftKey&&(e.preventDefault(),C())}).on(`click`,`.fl_act_copy`,function(){let n=w(e(this).data(`id`));n&&t(n.contenido,this,`¡Copiado!`)}).on(`click`,`.fl_act_del`,function(){let t=e(this).data(`id`);confirm(`¿Eliminar este flash?`)&&(s=s.filter(e=>e.id!==t),p.set(s),e(this).closest(`.fl_card`).slideUp(200,function(){e(this).remove()}),a(`Flash eliminado`,`info`),i.user&&y(t))}).on(`click`,`.fl_act_pin`,function(){let t=w(e(this).data(`id`));t&&(t.pin=!t.pin,p.set(s),g(),a(t.pin?`Fijado ✓`:`Desanclado`,`success`),i.user&&v(t).then(()=>f(t.id)))}).on(`click`,`.fl_act_edit`,function(){let t=w(e(this).data(`id`));if(!t)return;let n=e(this).closest(`.fl_card`).find(`.fl_card_body`);n.html(`<textarea class="fl_edit_ta">${t.contenido}</textarea>
        ...
        </div>`);let r=n.find(`.fl_edit_ta`).focus();r.on(`input`,function(){this.style.height=`auto`,this.style.height=`${this.scrollHeight}px`}),n.find(`.fl_edit_cancel`).on(`click`,g),n.find(`.fl_edit_ok`).on(`click`,()=>{let e=r.val().trim();e&&(Object.assign(t,{contenido:e,actualizado:Date.now()}),p.set(s),g(),a(`Flash actualizado ✓`,`success`),i.user&&v(t).then(()=>f(t.id)))})}),o([`.fl_hero > *`,`.fl_timeline_tit`],60),g(),S=i.on(async t=>{e(`#fl_btn_refresh`).toggle(!!t),t?await x(!0):(localStorage.removeItem(u),s=p.get(),g())}),setTimeout(()=>r.focus(),300),console.log(`⚡ ${c} v13 · Flash OK`)},w=()=>{S?.(),e(document).off(`click keydown`,`#fl_input, #fl_btn_send, #fl_btn_refresh, .fl_act_del, .fl_act_pin, .fl_act_edit, .fl_act_copy`)};export{w as cleanup,C as init,x as render};