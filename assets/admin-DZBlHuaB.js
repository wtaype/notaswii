import{t as e}from"./vendor-BDh6mtVu.js";import{t}from"./wii-BOG14u5H.js";import{f as n,i as r,p as i}from"./widev-DTlDEmBw.js";import{E as a,h as o,w as s,x as c}from"./firebase-CCUuWaKQ.js";import{n as l}from"./firebase-B1jMBrFt.js";var u=()=>n(`wiSmile`),d=`amUsersTotal`,f=`amUsersPro`,p=`amOrgsTotal`,m=`amLessonsTotal`,h=()=>{let e=u();return!e||e.rol!==`admin`?`<div class="am_page"><div class="am_empty"><i class="fas fa-ban"></i> Acceso denegado.</div></div>`:`
  <div class="am_page">
  ...
  </div>`},g=async()=>{let t=u();!t||t.rol!==`admin`||(e(document).off(`.am`),v(),y(),e(document).on(`click.am`,`#am_btn_refresh`,function(){let t=e(this).find(`i`).addClass(`fa-spin`);y(!0).then(()=>{setTimeout(()=>t.removeClass(`fa-spin`),500)})}))},_=()=>{e(document).off(`.am`)};function v(){e(`#am_n_total`).text(n(d)||`—`),e(`#am_n_pro`).text(n(f)||`—`),e(`#am_n_org`).text(n(p)||`—`),e(`#am_n_les`).text(n(m)||`45`)}async function y(t=!1){try{let u=a(l,`smiles`);if(t||!n(d)){let t=(await o(u)).data().count;i(d,t),e(`#am_n_total`).text(t)}if(t||!n(f)){let t=(await o(c(u,s(`rol`,`in`,[`gestor`,`empresa`,`admin`])))).data().count;i(f,t),e(`#am_n_pro`).text(t)}if(t||!n(p)){let t=(await o(c(u,s(`rol`,`==`,`empresa`)))).data().count;i(p,t),e(`#am_n_org`).text(t)}t&&r(`Estadísticas sincronizadas`,`success`)}catch(e){console.error(`[Admin] Error cargando stats:`,e),t&&r(`Error sincronizando`,`error`)}}export{_ as cleanup,g as init,h as render};