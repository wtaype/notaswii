...
<div class="tb_wrap">
  <div class="tb_hero">
    ...
    <div class="tb_hero_right">
      <button class="tb_btn_refresh" id="tb_btn_refresh" style="display:none" ${c(`Actualizar`)}><i class="fas fa-rotate-right"></i></button>
...
</div>`,x=null,S=async()=>{let n=m.get(),r=null,s=``,u=()=>e(`#tb_grid`).html(`<div class="tb_skeleton"></div>`.repeat(3)),p=t=>e(`[data-id="${t}"] .tb_act_cloud`).removeClass(`fa-cloud-arrow-up tb_cloud_pen`).addClass(`fa-cloud tb_cloud_ok`).attr(`data-witip`,`En nube`),h=()=>e(`#tb_count`).text(`${n.length} idea${n.length===1?``:`s`} guardada${n.length===1?``:`s`}`),b=()=>[...n].sort((e,t)=>e.pin&&!t.pin?-1:!e.pin&&t.pin?1:(t.creado||0)-(e.creado||0)),S=e=>`
  ...
  `,C=async()=>{let e=b();await t(`#tb_grid`,e.length?e.map(S).join(``):`<div class="tb_empty"><i class="fas fa-magic"></i><h2>Tu tablero está vacío</h2><p>Añade tu primera nota visual colorida para comenzar a organizar tus ideas.</p></div>`,80),e.length&&o([`.tb_grid > *`],60),h()},w=t=>{s=t;let n=e(`#tb_modal_box`);if(e(`.tb_theme_dot`).removeClass(`active`),!t)n.addClass(`default-color`).removeAttr(`style`),e(`.tb_theme_dot[data-hex=""]`).addClass(`active`),e(`#tb_in_color`).val(`#cccccc`);else{n.removeClass(`default-color`).attr(`style`,`--card-color: ${t};`);let r=e(`.tb_theme_dot[data-hex="${t.toUpperCase()}"]`);r.length&&r.addClass(`active`),e(`#tb_in_color`).val(t)}},T=(t=null)=>{if(r=t,t){let r=n.find(e=>e.id===t);r&&(e(`#tb_in_tit`).val(r.titulo),e(`#tb_in_cnt`).val(r.contenido),w(r.color||``))}else e(`#tb_in_tit`).val(``),e(`#tb_in_cnt`).val(``),w(``);e(`#tb_modal_wrap`).addClass(`active`),setTimeout(()=>e(`#tb_in_cnt`).focus(),100)},E=()=>{e(`#tb_modal_wrap`).removeClass(`active`),r=null};e(document).on(`click`,`#tb_btn_add`,()=>T()).on(`click`,`#tb_btn_close`,E).on(`click`,`#tb_modal_wrap`,function(e){e.target===this&&E()}).on(`click`,`#tb_btn_refresh`,async function(){let t=e(this).find(`i`);if(t.hasClass(`tb_spin`))return;t.addClass(`tb_spin`);let r=await y();r&&(JSON.stringify(r)!==JSON.stringify(n)&&(n=r,m.set(n),C()),a(`Sincronizado ✓`,`success`)),t.removeClass(`tb_spin`)}).on(`click`,`.tb_theme_dot`,function(){w(e(this).data(`hex`))}).on(`input`,`#tb_in_color`,function(){w(e(this).val())}).on(`click`,`#tb_btn_save_item`,()=>{let t=e(`#tb_in_tit`).val().trim(),c=e(`#tb_in_cnt`).val().trim();if(!t&&!c){a(`La nota está vacía`,`warning`);return}if(r){let e=n.find(e=>e.id===r);e&&(e.titulo=t,e.contenido=c,e.color=s,e.creado=Date.now(),e.synced=!1),n=n.filter(e=>e.id!==r),n.unshift(e),m.set(n),a(`Nota actualizada`,`success`),i.logged&&_(e).then(()=>{e.synced=!0,m.set(n),p(e.id)})}else{n=n.filter(e=>!e.id.startsWith(`ej`));let e={id:f(),titulo:t,contenido:c,color:s,pin:!1,creado:Date.now(),synced:!1};n.unshift(e),m.set(n),a(`Nota creada`,`success`),i.logged&&g(e).then(()=>{e.synced=!0,m.set(n),p(e.id)})}C(),E(),o([`.tb_card:first-child`],0)}).on(`click`,`.tb_act_pin`,function(t){t.stopPropagation();let r=e(this).data(`id`),o=n.find(e=>e.id===r);o&&(o.pin=!o.pin,m.set(n),C(),a(o.pin?`Nota fijada ✓`:`Desanclada`,`success`),i.logged&&_(o).then(()=>p(r)))}).on(`click`,`.tb_btn_act.edit, .tb_card`,function(t){e(t.target).closest(`.tb_btn_act`).length&&!e(this).hasClass(`edit`)||(t.stopPropagation(),T(e(this).closest(`.tb_card`).data(`id`)))}).on(`click`,`.tb_btn_act.del`,function(t){if(t.stopPropagation(),!confirm(`¿Eliminar esta nota del tablero?`))return;let r=e(this).closest(`.tb_card`),o=r.data(`id`);n=n.filter(e=>e.id!==o),m.set(n),r.css({transform:`scale(0.8)`,opacity:0}),setTimeout(()=>{r.remove(),h(),n.length||C()},300),a(`Nota eliminada`,`success`),i.logged&&v(o)}),o([`.tb_hero_left`,`.tb_btn_new`],50),C(),x=i.on(async t=>{if(e(`#tb_btn_refresh`).toggle(!!t),t){n.length===0&&u();let e=await y();e&&JSON.stringify(e)!==JSON.stringify(n)&&(n=e,m.set(n),C())}else localStorage.removeItem(d),n=m.get(),C()}),console.log(`✅ ${l} v13 · Tablero OK`)},C=()=>{e(document).off(`click input`,`#tb_btn_add, #tb_btn_close, #tb_modal_wrap, #tb_btn_refresh, .tb_theme_dot, #tb_in_color, #tb_btn_save_item, .tb_act_pin, .tb_btn_act.edit, .tb_card, .tb_btn_act.del`),x?.()};export{C as cleanup,S as init,b as render};