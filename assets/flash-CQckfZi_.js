import{t as e}from"./vendor-BDh6mtVu.js";import{C as t,_ as n,a as r,f as i,h as a,i as o,m as s,p as c,x as l}from"./widev-H4r-I7mR.js";import{t as u}from"./wii-DG5HC90G.js";import{t as d}from"./preload-helper-DbWsiEEd.js";var f=`flash`,p=()=>`fl`+Date.now(),m={get:()=>{try{return JSON.parse(localStorage.getItem(f)||`[]`)}catch{return[]}},set:e=>localStorage.setItem(f,JSON.stringify(e))},h=()=>!!i(`fl_sync`),g=()=>c(`fl_sync`,1,168),_=e=>{let t=Math.floor((Date.now()-e)/1e3);if(t<60)return`Justo ahora`;let n=Math.floor(t/60);if(n<60)return`Hace ${n} min`;let r=Math.floor(n/60);return r<24?`Hace ${r}h`:new Date(e).toLocaleDateString(`es-PE`,{day:`2-digit`,month:`short`})},v=({id:e,pin:t,synced:n,creado:r,contenido:i})=>`
  ...
  <div class="fl_timeline">
    ...
    <div id="fl_grid" class="fl_grid">
      <div class="fl_skeleton"></div><div class="fl_skeleton"></div><div class="fl_skeleton"></div>
        ...
        </div>`);let r=n.find(`.fl_edit_ta`).focus();r.on(`input`,function(){this.style.height=`auto`,this.style.height=`${this.scrollHeight}px`}),n.find(`.fl_edit_cancel`).on(`click`,f),n.find(`.fl_edit_ok`).on(`click`,()=>{let e=r.val().trim();e&&(Object.assign(t,{contenido:e,actualizado:Date.now()}),m.set(i),f(),o(`Flash actualizado ✓`,`success`),a.logged&&x(t).then(()=>l(t.id)))})}),s([`.fl_hero > *`,`.fl_timeline_tit`],60),i.length?(f(),a.logged&&!h()&&_(!1)):a.logged?await _(!0):f(),setTimeout(()=>r.focus(),300),console.log(`⚡ ${u} v11 · Flash OK`)},E=()=>{e(document).off(`click keydown`,`#fl_input, #fl_btn_send, #fl_btn_refresh, .fl_act_del, .fl_act_pin, .fl_act_edit, .fl_act_copy`)};export{E as cleanup,T as init,w as render};