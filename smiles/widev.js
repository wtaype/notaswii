...
// SHOWI V10 _________________________________
export const showi = (() => {
  let c;
  return (s, st = 45) => {
    if (!c) c = $('head').append(`<style>.swi{opacity:0;transform:translateY(3vh);transition:all .7s cubic-bezier(.4,0,.2,1)}</style>`);
    const els = [].concat(s).flatMap(x => [...document.querySelectorAll(x)]).filter(e => !e.dataset.i);
    els.forEach(e => { e.dataset.i = 1; e.classList.add('swi'); });
    let n = 0, t;
    const o = new IntersectionObserver(es => {
      es.filter(e => e.isIntersecting).forEach(e => {
        setTimeout(() => {
          ...
          setTimeout(() => { e.target.classList.remove('swi'); e.target.style.opacity = ''; e.target.style.transform = ''; }, 750);
        }, n++ * st);
        o.unobserve(e.target);
      });
      clearTimeout(t); t = setTimeout(() => n = 0, 100);
    });
