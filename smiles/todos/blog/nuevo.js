    ...
    <form id="nu_form" autocomplete="off"><div class="nu_layout">
      <div class="nu_left">
        <div class="nu_card">
          ...
          <input id="nu_titulo" type="text" class="nu_titulo_inp" placeholder="${desc}" maxlength="100" required/>
          <div class="nu_slug_box">
            <span class="nu_slug_label"><i class="fas fa-link"></i> ${id}.web.app/</span>
...
export const init = async () => {
  ...
  // ── CARGAR SUGERENCIAS DE DB ────────────────────────────────
  const cargarSugerencias = async () => {
    try {
      const q = query(collection(db, COL), orderBy('creado', 'desc'), limit(15));
      ...
      if (setCats.size > 0) {
        $('#nu_cat_sug').html(Array.from(setCats).map(c => `<span class="nu_sug_chip cat_sug">${c}</span>`).join(''));
      }
      if (setTags.size > 0) {
        $('#nu_tag_sug').html(Array.from(setTags).map(t => `<span class="nu_sug_chip tag_sug">#${t}</span>`).join(''));
      }
  ...
  $('#nu_form').on('submit', async function(e){
    ...
    const [titulo,resumen,cat,img,contenido,slug] = [$('#nu_titulo').val().trim(),$('#nu_resumen').val().trim(),catVal,$('#nu_img').val().trim(),$('#nu_contenido').val().trim(),$('#nu_slug_inp').val().trim()];
