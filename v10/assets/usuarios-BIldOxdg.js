import{t as e}from"./vendor-BDh6mtVu.js";import{d as t,i as n,p as r,s as i}from"./widev-BY9EW4md.js";import{D as a,E as o,_ as s,m as c,v as l,x as u}from"./firebase-CCUuWaKQ.js";import{n as d}from"./firebase-BNeWaP4M.js";var f=()=>r(`wiSmile`),p=[],m=`todos`,h=()=>{let e=f();return!e||e.rol!==`admin`?`<div class="adu_page"><div class="adu_empty"><i class="fas fa-ban"></i><p>Acceso denegado.</p></div></div>`:`
  <div class="adu_page">
  ...
  </div>`},g=async()=>{let t=f();!t||t.rol!==`admin`||(e(document).off(`.adu`),await v(),e(document).on(`input.adu`,`#adu_input_search`,function(){y()}),e(document).on(`click.adu`,`.adu_filter_btn`,function(){e(`.adu_filter_btn`).removeClass(`active`),e(this).addClass(`active`),m=e(this).data(`rol`),y()}),e(document).on(`click.adu`,`#adu_btn_sync`,async function(){let t=e(this).find(`i`).addClass(`fa-spin`);await v(!0),setTimeout(()=>t.removeClass(`fa-spin`),500)}),e(document).on(`click.adu`,`.adu_btn_ico.danger`,async function(){let t=e(this).data(`id`);if(confirm(`¿Atención! Eliminarás a "${t}" de forma permanente. ¿Continuar?`))try{await c(a(d,`smiles`,t)),n(`Usuario eliminado`,`info`),await v(!0)}catch{n(`Error al eliminar`,`error`)}}),e(document).on(`click.adu`,`.adu_btn_ico.view`,function(){let t=e(this).data(`id`),n=p.find(e=>e.id===t);n&&b(n)}),e(document).on(`click.adu`,`.adu_modal_close, .adu_btn_cancel`,()=>e(`#adu_modales`).html(``)),e(document).on(`click.adu`,`.adu_modal_bg`,t=>{e(t.target).hasClass(`adu_modal_bg`)&&e(`#adu_modales`).html(``)}))},_=()=>{e(document).off(`.adu`)};async function v(t=!1){try{p=(await s(u(o(d,`smiles`),l(300)))).docs.map(e=>({id:e.id,usuario:e.id,...e.data()})),y()}catch(t){console.error(`[admin_usuarios] Error:`,t),e(`#adu_table_body`).html(`<tr><td colspan="5"><div class="adu_empty" style="padding:4vh"><i class="fas fa-exclamation-triangle"></i><p>Error de conexión.</p></div></td></tr>`)}}function y(){let n=(e(`#adu_input_search`).val()||``).toLowerCase(),r=p.filter(e=>!(m!==`todos`&&(e.rol||`smile`)!==m||n&&![e.id,e.email,e.nombre,e.nombres,e.apellidos].join(` `).toLowerCase().includes(n)));if(!r.length){e(`#adu_table_body`).html(`<tr><td colspan="5"><div class="adu_empty" style="padding:4vh"><i class="fas fa-user-slash"></i><p>No se encontraron usuarios.</p></div></td></tr>`);return}let a=r.map(e=>{let n=e.nombres||e.nombre||e.id,r=e.email||`—`,a=i(n),o=e.rol||`smile`,s=e.fecha?.toDate?t(e.fecha):`—`;return`
      <tr class="adu_row">
        <td>
          <div class="adu_user_cell">
            <div class="adu_av">${a}</div>
            <div>
              <div class="adu_nom">${n}</div>
              <div class="adu_eml">${r}</div>
            </div>
          </div>
        </td>
        <td>
