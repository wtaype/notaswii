import{t as e}from"./vendor-BDh6mtVu.js";import{S as t,a as n,g as r,h as i,i as a,m as o,p as s,v as c}from"./widev-BY9EW4md.js";import{t as l}from"./wii-IZM0MjcA.js";import{t as u}from"./preload-helper-CwAw_xRM.js";var d=`tablero_items`,f=()=>`tb`+Date.now(),p={get:()=>{try{return JSON.parse(localStorage.getItem(d)||`[]`)}catch{return[]}},set:e=>localStorage.setItem(d,JSON.stringify(e))},m=()=>!!s(`tb_sync`),h=()=>o(`tb_sync`,1,168),g=async()=>{let{db:e}=await u(async()=>{let{db:e}=await import(`./firebase-BNeWaP4M.js`).then(e=>e.r);return{db:e}},[]);return{db:e,...await u(()=>import(`./firebase-CCUuWaKQ.js`).then(e=>e.f),[])}},_=async e=>{let t=r.user;if(t?.usuario)try{let{db:n,doc:r,setDoc:i,serverTimestamp:a}=await g();await i(r(n,`tableroNotas`,e.id),{id:e.id,usuario:t.usuario,email:t.email,titulo:String(e.titulo||``),contenido:String(e.contenido||``),color:String(e.color||``),pin:!!e.pin,creado:a(),actualizado:a()})}catch(e){console.error(`[tablero] guardarNube:`,e)}},v=async e=>{if(r.user?.usuario)try{let{db:t,doc:n,updateDoc:r,serverTimestamp:i}=await g();await r(n(t,`tableroNotas`,e.id),{titulo:String(e.titulo||``),contenido:String(e.contenido||``),color:String(e.color||``),pin:!!e.pin,actualizado:i()})}catch(e){console.error(`[tablero] actualizarNube:`,e)}},y=async e=>{if(r.user?.usuario)try{let{db:t,doc:n,deleteDoc:r}=await g();await r(n(t,`tableroNotas`,e))}catch{}},b=async()=>{let e=r.user;if(!e?.email)return null;try{let{db:t,collection:n,getDocs:r,query:i,where:a}=await g();return(await r(i(n(t,`tableroNotas`),a(`email`,`==`,e.email)))).docs.map(e=>{let t=e.data();return{id:e.id,titulo:t.titulo||``,contenido:t.contenido||``,color:t.color||``,pin:!!t.pin,creado:t.creado?.toMillis?.()||Date.now(),synced:!0}})}catch{return null}},x=()=>`
<div class="tb_wrap">
  <div class="tb_hero">
    <div class="tb_hero_left">
      <h1><i class="fas fa-th-large"></i> ${r.user?`${n()}${r.user.nombre||r.user.usuario}`:`Tu Tablero Visual`}</h1>
      <p id="tb_count">0 ideas guardadas</p>
    </div>
    <div class="tb_hero_right">
      <button class="tb_btn_refresh" id="tb_btn_refresh" ${t(`Actualizar`)}><i class="fas fa-rotate-right"></i></button>
...
</div>`,S=async()=>{let n=p.get(),o=null,s=``,u=()=>e(`#tb_grid`).html(`<div class="tb_skeleton"></div>`.repeat(3)),d=t=>e(`[data-id="${t}"] .tb_act_cloud`).removeClass(`fa-cloud-arrow-up tb_cloud_pen`).addClass(`fa-cloud tb_cloud_ok`).attr(`data-witip`,`En nube`),g=()=>e(`#tb_count`).text(`${n.length} idea${n.length===1?``:`s`} guardada${n.length===1?``:`s`}`),x=()=>[...n].sort((e,t)=>e.pin&&!t.pin?-1:!e.pin&&t.pin?1:(t.creado||0)-(e.creado||0)),S=e=>`
  ...
  `,C=async()=>{let e=x();await c(`#tb_grid`,e.length?e.map(S).join(``):`<div class="tb_empty"><i class="fas fa-magic"></i><h2>Tu tablero está vacío</h2><p>Añade tu primera nota visual colorida para comenzar a organizar tus ideas.</p></div>`,80),e.length&&i([`.tb_grid > *`],60),g()},w=async(t=!1)=>{if(!r.logged)return;t&&u();let i=e(`#tb_btn_refresh i`).addClass(`tb_spin`);try{let e=await b();if(e?.length){let t=new Set(e.map(e=>e.id)),r=n.filter(e=>!t.has(e.id));r.forEach(e=>_(e)),n=[...e,...r],p.set(n),h()}}finally{i.removeClass(`tb_spin`),C()}},T=t=>{s=t;let n=e(`#tb_modal_box`);if(e(`.tb_theme_dot`).removeClass(`active`),!t)n.addClass(`default-color`).removeAttr(`style`),e(`.tb_theme_dot[data-hex=""]`).addClass(`active`),e(`#tb_in_color`).val(`#cccccc`);else{n.removeClass(`default-color`).attr(`style`,`--card-color: ${t};`);let r=e(`.tb_theme_dot[data-hex="${t.toUpperCase()}"]`);r.length&&r.addClass(`active`),e(`#tb_in_color`).val(t)}},E=(t=null)=>{if(o=t,t){let r=n.find(e=>e.id===t);r&&(e(`#tb_in_tit`).val(r.titulo),e(`#tb_in_cnt`).val(r.contenido),T(r.color||``))}else e(`#tb_in_tit`).val(``),e(`#tb_in_cnt`).val(``),T(``);e(`#tb_modal_wrap`).addClass(`active`),setTimeout(()=>e(`#tb_in_cnt`).focus(),100)},D=()=>{e(`#tb_modal_wrap`).removeClass(`active`),o=null};e(document).on(`click`,`#tb_btn_add`,()=>E()).on(`click`,`#tb_btn_close`,D).on(`click`,`#tb_modal_wrap`,function(e){e.target===this&&D()}).on(`click`,`#tb_btn_refresh`,()=>w(!0)).on(`click`,`.tb_theme_dot`,function(){T(e(this).data(`hex`))}).on(`input`,`#tb_in_color`,function(){T(e(this).val())}).on(`click`,`#tb_btn_save_item`,()=>{let t=e(`#tb_in_tit`).val().trim(),c=e(`#tb_in_cnt`).val().trim();if(!t&&!c){a(`La nota está vacía`,`warning`);return}if(o){let e=n.find(e=>e.id===o);e&&(e.titulo=t,e.contenido=c,e.color=s,e.creado=Date.now(),e.synced=!1),n=n.filter(e=>e.id!==o),n.unshift(e),p.set(n),a(`Nota actualizada`,`success`),r.logged&&v(e).then(()=>{e.synced=!0,p.set(n),d(e.id)})}else{let e={id:f(),titulo:t,contenido:c,color:s,pin:!1,creado:Date.now(),synced:!1};n.unshift(e),p.set(n),a(`Nota creada`,`success`),r.logged&&_(e).then(()=>{e.synced=!0,p.set(n),d(e.id)})}C(),D(),i([`.tb_card:first-child`],0)}).on(`click`,`.tb_act_pin`,function(t){t.stopPropagation();let i=e(this).data(`id`),o=n.find(e=>e.id===i);o&&(o.pin=!o.pin,p.set(n),C(),a(o.pin?`Nota fijada ✓`:`Desanclada`,`success`),r.logged&&v(o).then(()=>d(i)))}).on(`click`,`.tb_btn_act.edit, .tb_card`,function(t){e(t.target).closest(`.tb_btn_act`).length&&!e(this).hasClass(`edit`)||(t.stopPropagation(),E(e(this).closest(`.tb_card`).data(`id`)))}).on(`click`,`.tb_btn_act.del`,function(t){if(t.stopPropagation(),!confirm(`¿Eliminar esta nota del tablero?`))return;let i=e(this).closest(`.tb_card`),o=i.data(`id`);n=n.filter(e=>e.id!==o),p.set(n),i.css({transform:`scale(0.8)`,opacity:0}),setTimeout(()=>{i.remove(),g(),n.length||C()},300),a(`Nota eliminada`,`success`),r.logged&&y(o)}),i([`.tb_hero_left`,`.tb_btn_new`],50),n.length?(C(),r.logged&&!m()&&w(!1)):r.logged?await w(!0):C(),console.log(`✅ ${l} v10 · Tablero OK`)},C=()=>{e(document).off(`click input`,`#tb_btn_add, #tb_btn_close, #tb_modal_wrap, #tb_btn_refresh, .tb_theme_dot, #tb_in_color, #tb_btn_save_item, .tb_act_pin, .tb_btn_act.edit, .tb_card, .tb_btn_act.del`)};export{C as cleanup,S as init,x as render};